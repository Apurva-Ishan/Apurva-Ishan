...
buttons.forEach(function (button) {
  ...
  button.addEventListener('click', function (e) {
    ...
    if (e.target.id === 'gothic') {
      ...
      const addText = document.createTextNode("Gothic")
      colorTitle.style.color = '#38494f';
      colorTitle.appendChild(addText)
      const addFact = document.createTextNode('Fun Fact: While red is often used to get attention, this subdued, grayish-blue is often used to recede into the background, making it an excellent, professional, and sophisticated choice for modern interior design or clothing to create a minimalist, relaxing vibe.')
    ...
    if (e.target.id === 'apricot') {
      ...
      const addText = document.createTextNode("Apricot")
      colorTitle.style.color = '#402222';
      colorTitle.appendChild(addText)
      const addFact = document.createTextNode('Fun Fact: As a warm, light color, this color is perceived as being closer to the viewer and larger than its actual size, which is useful in design to create a sense of intimacy.')
    ...
    if (e.target.id === 'sundance') {
      ...
      const addText = document.createTextNode("Sundance")
      colorTitle.style.color = '#19160b';
      colorTitle.appendChild(addText)
      const addFact = document.createTextNode('Fun Fact: In color psychology, this type of dull, greenish-yellow is sometimes associated with wisdom, intellect, and sometimes jealousy or deceit, depending on the context.')
    ...
    if (e.target.id === 'sapphire') {
      ...
      const addText = document.createTextNode("Sapphire")
      colorTitle.style.color = '#171f44';
      colorTitle.appendChild(addText)
      const addFact = document.createTextNode('Fun Fact: While some shades of blue are considered sad, this deep, rich tone is often associated with calming the nervous system and boosting creativity.')
